...
class TestIsolamentoPlanoContas:
    ...
    def test_sessao_sem_fazenda_e_recusada(self, client):
        ...
        c, _ = client
        _como_fazenda(1)
        c.post("/financeiro/plano-contas", json={"codigo": "9.96", "nome": "Fazenda 1"})
        _como_fazenda(2)
        c.post("/financeiro/plano-contas", json={"codigo": "9.95", "nome": "Fazenda 2"})

        _como_fazenda(None)
        r = c.get("/financeiro/plano-contas")
        assert r.status_code == 409, (
            f"o plano de contas das duas fazendas saiu junto para uma sessão sem fazenda "

...
# ---------------------------------------------------------------------------
# "Prova ao vivo" — Estoque de Sêmen > Prova média (Rebanho > Touros). Ao
# contrário da prova média genética (índices/PTA do catálogo, ver
# fazenda.rules.touros.calcular_prova_media), esta é a taxa de concepção
# REALIZADA no próprio rebanho, por touro/sêmen — o que de fato aconteceu ao
# usar aquele sêmen, não o índice de catálogo dele.
# ---------------------------------------------------------------------------
def prova_ao_vivo_por_touro(
    registros: list[dict], *, hoje: date | None = None,
    categoria: str | None = None, ano_nascimento: int | None = None,
    periodo_de: date | None = None, periodo_ate: date | None = None,
    dias_resultado: int = DIAS_RESULTADO_CONHECIDO_PADRAO,
) -> list[dict]:
    """
    Filtros (categoria, ano de nascimento da matriz, período da inseminação)
    são todos opcionais e só LIMITAM o resultado. A regra de "resultado já
    conhecido" (R7, `conta_em_taxa`) usa a data da IA MAIS RECENTE de cada
    animal calculada sobre o histórico COMPLETO (antes de aplicar os
    filtros) — um filtro de período não pode esconder a IA mais nova que
    prova o desfecho de uma IA mais antiga do mesmo animal.
    """
    hoje = hoje or date.today()
    ultimas_datas: dict[str, date] = {}
    for r in registros:
    ...
    def _passa_filtro(r: dict) -> bool:
        if r["touro"] == "(sem touro)":
            return False
        if categoria and categoria != "todas" and (r["categoria"] or "").strip().lower() != categoria.strip().lower():
            return False
    ...
    por_touro: dict[str, list[dict]] = defaultdict(list)
    for r in registros:
        if _passa_filtro(r):
            por_touro[r["touro"]].append(r)

    saida = []
    for touro, regs in por_touro.items():
        contam = []
        for r in regs:
            d = r["data_servico"]
            if d is None:
                continue
            posterior = bool((ultima := ultimas_datas.get(r["numero"])) and ultima > d)
            if conta_em_taxa(r, hoje, servico_posterior=posterior, dias=dias_resultado):
                contam.append(r)
        positivos = sum(1 for r in contam if r["positivo"])
        taxa = round(100 * positivos / len(contam), 1) if contam else None
        saida.append({
            "touro": touro, "servicos": len(regs), "elegiveis": len(contam),
            "positivos": positivos, "taxa_concepcao": taxa,
        })
    # Melhor taxa primeiro; sem nenhum resultado conhecido ainda, vai pro fim.
    saida.sort(key=lambda x: (x["taxa_concepcao"] is None, -(x["taxa_concepcao"] or 0)))
    return saida

...
def eh_membro_equipe_cowdata(session: Session, user: Usuario) -> bool:
    ...
    `session` (RLS-bound) É IGNORADO DE PROPÓSITO para as duas leituras
    abaixo — incidente de 11/09/2026: isto é uma checagem de IDENTIDADE
    (a Pessoa deste usuário mora na fazenda interna da CowData, quase nunca
    a fazenda hoje selecionada na sessão — no login, nenhuma ainda está
    selecionada), não um dado da fazenda atual. Sob RLS, ler pela `session`
    da requisição nega isto em silêncio sempre que a fazenda interna da
    CowData não é a do contexto corrente — foi o que derrubou a tela de
    login/seleção de fazenda em produção. `usuario_id`/`pessoa_id` já são o
    recorte de segurança real aqui (mesmo raciocínio das rotinas de fundo,
    ver roteiro-seguranca.md seção 2) — ler pela conexão de dono
    (`engine_manutencao`) é seguro e necessário."""
    if not user.pessoa_id:
        return False
    with Session(engine_manutencao) as s:
...
def eh_consultor_cowdata(session: Session, usuario: Usuario) -> bool:
    ...
    Mesma razão de `eh_membro_equipe_cowdata` para ignorar a `session`
    passada e ler pela `engine_manutencao`: a Pessoa deste usuário mora na
    fazenda interna da CowData, não na fazenda-cliente hoje selecionada (é
    justamente aqui, em exigir_admin_ou_consultor_fazenda, que as duas

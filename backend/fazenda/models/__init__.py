"""
Camada de dados — modelos SQLModel (tabelas SQLite/PostgreSQL + validação Pydantic).
Cada modelo representa uma entidade do domínio da fazenda.

Este pacote foi dividido em submódulos por domínio (ver arquivos abaixo) para
facilitar a navegação — este __init__.py é só um shim de re-export: importa
todas as classes de todos os submódulos para que `from fazenda.models import X`
continue funcionando exatamente como antes da divisão, e para garantir que
todas as tabelas fiquem registradas no SQLModel.metadata/registry antes de
qualquer query (import eager de todos os submódulos).

NOTA: Relacionamentos usam strings forward-reference para compatibilidade
com SQLModel 0.0.39 + SQLAlchemy 2.x (sem Mapped[] em modelos com table=True).
"""
from __future__ import annotations

from sqlmodel import SQLModel

from .animais import (
    Animal,
    Lote,
    MovimentoLote,
    MotivoBaixa,
    MotivoVenda,
    Raca,
    GrauSangue,
    MotivoMovimentacao,
    ParametroSugestaoMovimentacao,
    BaixaAnimal,
    CompraAnimal,
    VendaAnimal,
    ComissaoCorretagem,
)
from .reprodutivo import (
    Servico,
    TipoServicoReprodutivo,
    MetodoServicoReprodutivo,
    ProtocoloIatf,
    ProtocoloIatfEtapa,
    ProtocoloIatfLancamento,
    ProtocoloIatfAplicacao,
    ProtocoloIatfHormonio,
    Parto,
    ColostragemBezerra,
)
from .producao import (
    ControleLeiteiro,
    PesagemCorporal,
    AgendamentoPesagem,
    QualidadeLeite,
    EntregaLeiteMensal,
    Secagem,
    Lactacao,
    FaixaBonificacaoQualidade,
)
from .financeiro import (
    ContaGerencial,
    LancamentoItem,
    LancamentoAnexo,
    PlanoContaGerencial,
    ContaCorrente,
    TransferenciaContas,
    CentroCusto,
    TipoDocumento,
    FormaPagamentoCadastro,
    ClassificacaoLancamento,
    OrcamentoItem,
    PlanejamentoCenario,
    PlanejamentoItem,
    Pedido,
    PedidoItem,
    PedidoAnexo,
    CATEGORIAS_PEDIDO_ANEXO,
    Patrimonio,
    ManutencaoPatrimonio,
    CurvaABC,
    LancamentoRecorrente,
    CartaoCredito,
    FaturaCartao,
    LancamentoCartao,
)
from .estoque import (
    Estoque,
    Fornecedor,
    FornecedorClienteApelido,
    MovimentoEstoque,
    EstoqueSemen,
    CompraSemen,
    EstoquePrincipioAtivo,
    EstoqueAliasMesclado,
    LoteEstoque,
    ApresentacaoEmbalagemEstoque,
    LocalArmazenamento,
    CategoriaEstoque,
    FinalidadeEstoque,
    UnidadeEstoque,
    UnidadeEmbalagemEstoque,
    UnidadeMedidaEmbalagemEstoque,
)
from .agricultura import (
    Safra,
)
from .folha_rubrica import (
    FolhaRubrica,
)
from .pessoal import (
    TipoPessoa,
    Pessoa,
    PessoaAnexo,
    CATEGORIAS_PESSOA_ANEXO,
    FolhaPagamento,
    FeriasFuncionario,
    DecimoTerceiro,
    RescisaoFuncionario,
    ValeFuncionario,
    ValeParcela,
    ValeAvulso,
    ValeAvulsoAbatimento,
    Empreitada,
    EmpreitadaParcela,
    EmpreitadaEtapa,
    Contrato,
    ContratoParcela,
    Diaria,
    DiariaPagamento,
    DiariaAuditoria,
    DiariaDia,
    ParametroDiariaPadrao,
    GuiaFolhaEncargo,
)
from .sanidade import (
    Sanidade,
    AplicacaoAgendada,
    PrincipioAtivo,
    ParametroMinimoFarmacia,
    MedicamentoComercial,
    Doenca,
    IndicacaoTerapeutica,
    MedicamentoPrincipioAtivo,
    Laboratorio,
    CategoriaMedicamento,
    ClassificacaoMedicamento,
    MedicamentoCategoria,
    MedicamentoClassificacao,
    EstoqueCategoriaMedicamento,
    EstoqueClassificacaoMedicamento,
    ExameDefinicao,
    ExameResultado,
    EventoSanitario,
    CalendarioSanitario,
    CronogramaSanitario,
    CronogramaSanitarioAnimal,
    ChecklistTemplateItem,
    ChecklistItem,
    CalendarioSanitarioChecklistItem,
    ServicoCadastro,
    ProtocoloSanitario,
    ProtocoloSanitarioEtapa,
    ProtocoloSanitarioLote,
    ProtocoloSanitarioLancamento,
    ProtocoloSanitarioAplicacao,
    ProtocoloInducaoLactacao,
    ProtocoloInducaoLactacaoEtapa,
    ProtocoloInducaoLancamento,
    ProtocoloInducaoMedicamento,
    ProtocoloInducaoAplicacao,
    Touro,
)
from .alimentacao import (
    CategoriaAlimento,
    Alimento,
    Dieta,
    DietaLancamento,
    DietaItemProgramado,
    ConsumoAlimento,
    ConsumoSobra,
    IngredienteMS,
    TabelaNutricionalProduto,
    TabelaNutricionalValor,
    AnaliseBromatologica,
    DietaRegistroReal,
    AlimentacaoEstado,
)
from .formulacao import (
    AlimentoNutricional,
    DietaSimulacao,
    DietaSimulacaoItem,
)
from .recria import (
    OcorrenciaClinica,
    MetaRecria,
    PesoAlvoIdade,
    FaseRecria,
    JanelaPontoCritico,
    BenchmarkRecria,
    RegistroCocho,
    CategoriaManejo,
)
from .multitenant import (
    Fazenda,
    UsuarioFazenda,
    EmpresaOperadora,
)
from .cotacao import (
    Cotacao,
    CotacaoItem,
    CotacaoFornecedor,
    CotacaoResposta,
    PedidoConfirmacao,
    FornecedorCategoria,
)
from .catalogo_cowdata import (
    ClassificacaoCowData,
    FinalidadeCowData,
    FornecedorCowData,
    FornecedorCowDataClassificacao,
    FornecedorCowDataFinalidade,
    ProdutoPadrao,
    ProdutoPadraoFinalidade,
    CotacaoCowData,
    CotacaoCowDataFornecedor,
    CotacaoCowDataItem,
    CotacaoCowDataResposta,
    PrecoBaseSugerido,
    PrecoBaseSugeridoParticipanteMedia,
)
from .planos import (
    PrecoModulo,
    ContratoFazenda,
    ContratoFazendaModulo,
    ContratoAnexo,
    ContratoAssinaturaZapSign,
)
from .cobranca import (
    CobrancaBoleto,
    CobrancaPix,
    CobrancaAsaas,
)
from .cowdata_interno import (
    LancamentoCowData,
)
from .cofre_acesso import (
    PedidoAcessoSuporte,
    SessaoAcessoSuporte,
    AuditoriaAcessoSuporte,
    AcaoAuditoriaSuporte,
)
from .equipe_cowdata_acesso import (
    PermissaoEquipeCowData,
)
from .documentos import (
    DocumentoArquivado,
    Chamado,
)
from .fotos import (
    FotoCampo,
)
from .fotos_news import (
    PastaFotoNews,
    FotoNews,
)
from .alerta_indicador import (
    AlertaIndicador,
)
from .onboarding import (
    OnboardingUsuario,
)
from .filtro_salvo import (
    FiltroSalvo,
)
from .protocolo_customizado import (
    ProtocoloCustomizado,
    ProtocoloCustomizadoEtapa,
    ProtocoloCustomizadoLancamento,
    ProtocoloCustomizadoAplicacao,
)
from .lida import (
    Lida,
    LidaEtapa,
    LidaLancamento,
    LidaAplicacao,
)
from .idempotencia import (
    IdempotenciaChave,
)
from .sistema import (
    SeedFlag,
    ParametroFazenda,
    TelegramPendente,
    TelegramSessao,
    LancamentoPendente,
    AgendaManual,
    EventoRealizado,
    SolicitacaoExclusao,
    Usuario,
    LoginAcesso,
    BackupAutomatico,
    FonteNews,
    NoticiaNews,
    NotaCapa,
    PushSubscription,
    PushTokenFcm,
    PushNotificacaoEnviada,
    PortalMensagem,
    ParametroManualFazenda,
    SugestaoManualFazenda,
    AssistenteEnsinamento,
)

__all__ = [
    "Fazenda",
    "UsuarioFazenda",
    "EmpresaOperadora",
    "Cotacao",
    "CotacaoItem",
    "CotacaoFornecedor",
    "CotacaoResposta",
    "PedidoConfirmacao",
    "FornecedorCategoria",
    "ClassificacaoCowData",
    "FinalidadeCowData",
    "FornecedorCowData",
    "FornecedorCowDataClassificacao",
    "FornecedorCowDataFinalidade",
    "ProdutoPadrao",
    "ProdutoPadraoFinalidade",
    "CotacaoCowData",
    "CotacaoCowDataFornecedor",
    "CotacaoCowDataItem",
    "CotacaoCowDataResposta",
    "PrecoBaseSugerido",
    "PrecoBaseSugeridoParticipanteMedia",
    "PrecoModulo",
    "ContratoFazenda",
    "ContratoFazendaModulo",
    "ContratoAnexo",
    "ContratoAssinaturaZapSign",
    "CobrancaBoleto",
    "CobrancaPix",
    "DocumentoArquivado",
    "Chamado",
    "FotoCampo",
    "PastaFotoNews",
    "FotoNews",
    "AlertaIndicador",
    "OnboardingUsuario",
    "FiltroSalvo",
    "ProtocoloCustomizado",
    "ProtocoloCustomizadoEtapa",
    "ProtocoloCustomizadoLancamento",
    "IdempotenciaChave",
    "ProtocoloCustomizadoAplicacao",
    "Lida",
    "LidaEtapa",
    "LidaLancamento",
    "LidaAplicacao",
    "Animal",
    "Lote",
    "MovimentoLote",
    "MotivoBaixa",
    "MotivoVenda",
    "Raca",
    "GrauSangue",
    "MotivoMovimentacao",
    "ParametroSugestaoMovimentacao",
    "BaixaAnimal",
    "CompraAnimal",
    "VendaAnimal",
    "ComissaoCorretagem",
    "Servico",
    "TipoServicoReprodutivo",
    "MetodoServicoReprodutivo",
    "ProtocoloIatf",
    "ProtocoloIatfEtapa",
    "ProtocoloIatfLancamento",
    "ProtocoloIatfAplicacao",
    "ProtocoloIatfHormonio",
    "Parto",
    "ColostragemBezerra",
    "ControleLeiteiro",
    "PesagemCorporal",
    "AgendamentoPesagem",
    "QualidadeLeite",
    "EntregaLeiteMensal",
    "Secagem",
    "Lactacao",
    "FaixaBonificacaoQualidade",
    "ContaGerencial",
    "LancamentoItem",
    "LancamentoAnexo",
    "PlanoContaGerencial",
    "ContaCorrente",
    "TransferenciaContas",
    "CentroCusto",
    "TipoDocumento",
    "FormaPagamentoCadastro",
    "ClassificacaoLancamento",
    "OrcamentoItem",
    "PlanejamentoCenario",
    "PlanejamentoItem",
    "Pedido",
    "PedidoItem",
    "PedidoAnexo",
    "CATEGORIAS_PEDIDO_ANEXO",
    "Patrimonio",
    "ManutencaoPatrimonio",
    "Safra",
    "CurvaABC",
    "LancamentoRecorrente",
    "CartaoCredito",
    "FaturaCartao",
    "LancamentoCartao",
    "Estoque",
    "Fornecedor",
    "FornecedorClienteApelido",
    "MovimentoEstoque",
    "EstoqueSemen",
    "CompraSemen",
    "EstoquePrincipioAtivo",
    "EstoqueAliasMesclado",
    "LoteEstoque",
    "ApresentacaoEmbalagemEstoque",
    "LocalArmazenamento",
    "CategoriaEstoque",
    "FinalidadeEstoque",
    "UnidadeEstoque",
    "UnidadeEmbalagemEstoque",
    "UnidadeMedidaEmbalagemEstoque",
    "TipoPessoa",
    "Pessoa",
    "PessoaAnexo",
    "CATEGORIAS_PESSOA_ANEXO",
    "FolhaPagamento",
    "FolhaRubrica",
    "FeriasFuncionario",
    "DecimoTerceiro",
    "RescisaoFuncionario",
    "ValeFuncionario",
    "ValeParcela",
    "ValeAvulso",
    "Empreitada",
    "EmpreitadaParcela",
    "EmpreitadaEtapa",
    "Contrato",
    "ContratoParcela",
    "Diaria",
    "DiariaPagamento",
    "DiariaAuditoria",
    "DiariaDia",
    "ParametroDiariaPadrao",
    "GuiaFolhaEncargo",
    "Sanidade",
    "AplicacaoAgendada",
    "PrincipioAtivo",
    "ParametroMinimoFarmacia",
    "MedicamentoComercial",
    "Doenca",
    "IndicacaoTerapeutica",
    "MedicamentoPrincipioAtivo",
    "Laboratorio",
    "CategoriaMedicamento",
    "ClassificacaoMedicamento",
    "MedicamentoCategoria",
    "MedicamentoClassificacao",
    "EstoqueCategoriaMedicamento",
    "EstoqueClassificacaoMedicamento",
    "ExameDefinicao",
    "ExameResultado",
    "EventoSanitario",
    "CalendarioSanitario",
    "CronogramaSanitario",
    "CronogramaSanitarioAnimal",
    "ChecklistTemplateItem",
    "ChecklistItem",
    "CalendarioSanitarioChecklistItem",
    "ServicoCadastro",
    "ProtocoloSanitario",
    "ProtocoloSanitarioEtapa",
    "ProtocoloSanitarioLote",
    "ProtocoloSanitarioLancamento",
    "ProtocoloSanitarioAplicacao",
    "ProtocoloInducaoLactacao",
    "ProtocoloInducaoLactacaoEtapa",
    "ProtocoloInducaoLancamento",
    "ProtocoloInducaoMedicamento",
    "ProtocoloInducaoAplicacao",
    "Touro",
    "CategoriaAlimento",
    "Alimento",
    "Dieta",
    "DietaLancamento",
    "DietaItemProgramado",
    "ConsumoAlimento",
    "ConsumoSobra",
    "IngredienteMS",
    "TabelaNutricionalProduto",
    "TabelaNutricionalValor",
    "AnaliseBromatologica",
    "DietaRegistroReal",
    "AlimentacaoEstado",
    "AlimentoNutricional",
    "DietaSimulacao",
    "DietaSimulacaoItem",
    "OcorrenciaClinica",
    "MetaRecria",
    "PesoAlvoIdade",
    "FaseRecria",
    "JanelaPontoCritico",
    "BenchmarkRecria",
    "RegistroCocho",
    "CategoriaManejo",
    "SeedFlag",
    "ParametroFazenda",
    "TelegramPendente",
    "TelegramSessao",
    "LancamentoPendente",
    "AgendaManual",
    "EventoRealizado",
    "SolicitacaoExclusao",
    "Usuario",
    "LoginAcesso",
    "BackupAutomatico",
    "FonteNews",
    "NoticiaNews",
    "NotaCapa",
    "PushSubscription",
    "PushTokenFcm",
    "PushNotificacaoEnviada",
    "PortalMensagem",
    "ParametroManualFazenda",
    "SugestaoManualFazenda",
    "AssistenteEnsinamento",
]

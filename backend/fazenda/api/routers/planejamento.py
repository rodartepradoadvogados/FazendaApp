...
def listar_orcamento(ano: Optional[int] = Query(None), session: Session = Depends(get_session)) -> list[dict]:
    ...
    nomes = {c.codigo: c.nome for c in session.exec(select(PlanoContaGerencial)).all()}
...
def criar_item_orcamento(dados: OrcamentoItemIn, session: Session = Depends(get_session), user: Usuario = Depends(get_current_user)) -> dict:
...
def atualizar_item_orcamento(item_id: int, dados: OrcamentoItemIn, session: Session = Depends(get_session)) -> dict:
...
def excluir_item_orcamento(item_id: int, session: Session = Depends(get_session)) -> None:
    item = session.get(OrcamentoItem, item_id)
    if not item:
        raise HTTPException(status_code=404, detail="Item de orçamento não encontrado")
    session.delete(item)
...
@router.get("/cenarios")
def listar_cenarios(session: Session = Depends(get_session)) -> list[dict]:
    cenarios = session.exec(select(PlanejamentoCenario).where(PlanejamentoCenario.ativo == True).order_by(PlanejamentoCenario.criado_em.desc())).all()
    return [c.model_dump() for c in cenarios]


@router.post("/cenarios", status_code=201)
def criar_cenario(dados: CenarioIn, session: Session = Depends(get_session), user: Usuario = Depends(get_current_user)) -> dict:
    ...
    cenario = PlanejamentoCenario(nome=dados.nome, tipo=dados.tipo, observacao=dados.observacao, usuario_id=user.id if isinstance(user, Usuario) else None)
    session.add(cenario)
    session.commit()
...
def atualizar_cenario(cenario_id: int, dados: CenarioIn, session: Session = Depends(get_session)) -> dict:
...
def excluir_cenario(cenario_id: int, session: Session = Depends(get_session)) -> None:
    cenario = session.get(PlanejamentoCenario, cenario_id)
    if not cenario:
        raise HTTPException(status_code=404, detail="Cenário não encontrado")
...
def listar_itens_cenario(cenario_id: int, session: Session = Depends(get_session)) -> list[dict]:
    if not session.get(PlanejamentoCenario, cenario_id):
        raise HTTPException(status_code=404, detail="Cenário não encontrado")
    nomes = {c.codigo: c.nome for c in session.exec(select(PlanoContaGerencial)).all()}
...
def criar_item_cenario(cenario_id: int, dados: PlanejamentoItemIn, session: Session = Depends(get_session)) -> dict:
    if not session.get(PlanejamentoCenario, cenario_id):
        raise HTTPException(status_code=404, detail="Cenário não encontrado")
    if dados.tipo not in ("receita", "despesa"):
        raise HTTPException(status_code=400, detail="tipo deve ser 'receita' ou 'despesa'")
...
def atualizar_item_cenario(item_id: int, dados: PlanejamentoItemIn, session: Session = Depends(get_session)) -> dict:
...
def excluir_item_cenario(item_id: int, session: Session = Depends(get_session)) -> None:
    item = session.get(PlanejamentoItem, item_id)
    if not item:
        raise HTTPException(status_code=404, detail="Item de planejamento não encontrado")
    session.delete(item)
...
def projecao_cenario(cenario_id: int, session: Session = Depends(get_session)) -> dict:
...
def importar_para_pedido(dados: ImportarParaPedidoIn, session: Session = Depends(get_session), user: Usuario = Depends(get_current_user)) -> dict:
    ...
    numero_pedido = _proximo_numero_pedido(session, data_pedido.year)
    pedido = Pedido(

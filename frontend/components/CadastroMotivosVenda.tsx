...
import { Tag, Plus, Pencil, AlertTriangle, Check, X, Search } from "lucide-react";
...
export default function CadastroMotivosVenda() {
  ...
  const carregar = () => fetchMotivosVenda().then(setItens).catch((e) => setError(e.message));
  useEffect(() => { carregar(); }, []);
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (editando === "novo") await criarMotivoVenda(dados);
      else if (typeof editando === "number") await atualizarMotivoVenda(editando, dados);
      setEditando(null);
      await carregar();
    } catch (e: any) {
  ...
  return (
    <div className="card">
      <div className="card-header mb-3 flex items-center justify-between">
        <span className="flex items-center gap-2"><Tag size={16} /> Motivos de venda de animal</span>
      ...
      <p style={{ color: "var(--text-muted)", fontSize: "0.8rem", marginBottom: "0.8rem" }}>
        Motivo comercial da venda de um animal (Lançamentos &gt; Compra/Venda &gt; Vender animal). Desative em vez de
      ...
      {itens && (
        <>
          <div style={{ position: "relative", marginBottom: "0.8rem" }}>
            ...
            <input style={buscaInputStyle} value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar motivo de venda…" title="Buscar por nome" />
          </div>

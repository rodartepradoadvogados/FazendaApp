...
export function CowDataMark({
  ...
  size?: number | string;
  /** "escuro": tile ink com curva em gradiente ouro (padrão, lê bem sobre
   *  qualquer fundo). "claro": tile vinho, para compor com fundos claros/
   *  creme mantendo o mesmo contraste do tile escuro. "mono": sem tile, só a
   *  curva sólida numa cor (marca d'água, ou onde já existe um fundo). */
  ...
  const tileBg = variant === "claro" ? "#3A0F1A" : "#1E0F16";
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" aria-label="CowData" style={{ flexShrink: 0 }}>
      <defs>
        <linearGradient id={gradId} x1="0" y1="1" x2="1" y2="0">
          <stop offset="0%" stopColor="#B9831F" />
          <stop offset="100%" stopColor="#F0C874" />
        </linearGradient>
      </defs>
      <rect width="120" height="120" rx="28" fill={tileBg} />
      <rect x="4.5" y="4.5" width="111" height="111" rx="24" fill="none" stroke="#E0A63C" strokeOpacity="0.3" strokeWidth="1.3" />
      <line x1="16" y1="92" x2="104" y2="92" stroke="#7A2233" strokeWidth="1.4" />
      <path d={PATH_CURVA} fill="none" stroke={`url(#${gradId})`} strokeWidth="5.5" strokeLinecap="round" />
      <circle cx="58" cy="42.5" r="4.4" fill="#F3E7D3" />
      <circle cx="58" cy="42.5" r="7.6" fill="none" stroke="#E0A63C" strokeWidth="1.4" />
    </svg>
  );
}

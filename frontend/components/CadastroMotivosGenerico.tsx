...
export default function CadastroMotivosGenerico({
  ...
  textoVazio: string;
  fetchMotivos: () => Promise<Motivo[]>;
  criarMotivo: (dados: { nome: string; ativo?: boolean }) => Promise<unknown>;
  atualizarMotivo: (id: number, dados: { nome: string; ativo: boolean }) => Promise<unknown>;
}) {
  ...
  const carregar = () => fetchMotivos().then(setItens).catch((e) => setError(e.message));
  useEffect(() => { carregar(); }, []);
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (editando === "novo") await criarMotivo(dados);
      else if (typeof editando === "number") await atualizarMotivo(editando, dados);
      setEditando(null);
      await carregar();
    } catch (e: any) {
      ...
      {itens && (
        <>
          <div style={{ position: "relative", marginBottom: "0.8rem" }}>
            ...
            <input style={buscaInputStyle} value={busca} onChange={(e) => setBusca(e.target.value)} placeholder={placeholderBusca} title="Buscar por nome" />
          </div>
          ...
          <table className="fazenda-table">
            ...
            <tbody>
              ...
              {!itens.length && !editando && <tr><td colSpan={2} style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>{textoVazio}</td></tr>}

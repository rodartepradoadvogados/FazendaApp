  ...
  const entrar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.selecao_fazenda_necessaria) {
        // Mais de um acesso vinculado (fazendas e/ou Painel CowData) — a
        // escolha em si mora numa ÚNICA tela, reaproveitada aqui e por
        // "Trocar de conta" (ver components/EscolherConta.tsx e
        // app/escolher-conta/page.tsx para a justificativa de não duplicar
        // essa UI). Preserva o "next" original (ex.: /app no celular) para
        // a escolha de fazenda saber para onde ir depois.
        const next = new URLSearchParams(window.location.search).get("next");
        router.replace(`/escolher-conta${next ? `?next=${encodeURIComponent(next)}` : ""}`);
        return;
      }
      await irParaDestino();
    } catch (err: any) {
